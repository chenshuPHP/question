...
class Zhuanti_y2016 extends zhuanti_base {
	...
	public function handler(){
		$this->active->tpl_name = 'zhuanti/y2016/index.html';
	}
	...
	public function validate(){
		...
		$_SESSION['zhuanti_y2016_validate'] = $this->active->kaocode->getCode();
	...
	private function get_validate(){
		...
		session_start();
		$validate  = '';
		if( isset( $_SESSION['zhuanti_y2016_validate'] ) ){
			$validate = $_SESSION['zhuanti_y2016_validate'];
		}
		...
		if( count($errors) == 0 ){
			...
			try{
				$this->active->pub_model->express_pipe_add(array(
					'true_name'=>$data['name'],
					'tel'=>$data['mobile'],
					'rel'=>$this->active->encode->htmlencode( '{"url":"'. $zhuanti['link'] .'", "name":"2016新年优惠活动"}' )
				));
